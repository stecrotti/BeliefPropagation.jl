...
@testset "ksat random tree" begin
    ...
    @testset "Generic BP factor" begin
        ψ_generic = [BPFactor(bp.ψ[a], fill(2, degree(g, f_vertex(a)))) for a in eachfactor(g)]
        ϕ_generic = [BPFactor(bp.ϕ[i], (2,)) for i in eachvariable(g)]
...
@testset "ksat random graph" begin
    ...
    ψ = [KSATClause(bitrand(rng, degree(g, f_vertex(a)))) for a in eachfactor(g)]
    ϕ = [BPFactor(1.0 .+ 1e-4 * randn(2)) for _ in eachvariable(g)]
    bp = BP(g, ψ, fill(2, nvariables(g)); ϕ)
    test_za(bp)
    iterate!(bp; maxiter=100, tol=1e-12, rein=1e-2)
    xstar = argmax.(beliefs(bp))
    nunsat = sum(1 - Int(bp.ψ[a](xstar[i] for i in neighbors(bp.g, f_vertex(a)))) for a in eachfactor(bp.g))
end
